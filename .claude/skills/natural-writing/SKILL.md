...
20factor-analyser already has house rules in `skills.md` at the repo root.
Read both. Where they overlap, they agree; where `skills.md` is more
specific about this project, it wins.

The hardest audience here is the report copy. It is read by Indian school
children and their parents, and the words land on a child's handwriting.
Short words, concrete claims, no jargon. "Hand shaky" beats "tremor",
"writing flow" beats "rhythm". A coach tip a parent cannot act on is a
failed tip.

Two rules specific to a research project:

- Never invent a number. If OCR accuracy is 89%, print 89%, never "high
  accuracy". Every scored claim traces to code in `backend/` or
  `frontend/src/`.
- Deterministic CV over black-box AI is a deliberate product choice.
  Describe scoring in terms of what is measured, not what is "intelligent".

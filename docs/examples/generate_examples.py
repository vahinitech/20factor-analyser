...
def write_frontend_sample(result, lines):
    """Emit frontend/scripts/core/sample-report-data.js: the same
    example-report analysis, plus the line/word/letter counts of the page
    the scorer measured, in the shape sample-report.html feeds to
    VahiniReport.render. Written here so the docs JSON and the in-app
    sample can never drift apart."""
    ...
    path = os.path.join(
        ...
        "scripts",
        "core",
        "sample-report-data.js",
    )
    with open(path, "w", encoding="utf-8", newline="\n") as f:
        f.write(
            "/* Generated by docs/examples/generate_examples.py - do not"
            " edit by hand.\n"
            "   Real build_analysis output for the synthetic"
            " 'example-report' practice\n"
            "   page (docs/example-report.md tells its story);"
            " sample-report.html renders\n"
            "   it through the same VahiniReport renderer the live"
            " pipeline uses. */\n"
        )
        f.write("window.VAHINI_SAMPLE_REPORT = ")
        json.dump(payload, f, indent=2, ensure_ascii=False)
        f.write(";\n")


# The before/after scenario pairs case-studies.html shows; the stories
# around them live in docs/case-studies.md and in the page itself.
CASE_PAIRS = [
    ("case1", "case1-baseline-before", "case1-baseline-after"),
    ("case2", "case2-spacing-before", "case2-spacing-after"),
    ("case3", "case3-size-margin-before", "case3-size-margin-after"),
]


def _trim_for_cases(result):
    """The slice of an analysis case-studies.html compares: overall,
    per-factor scores/bands, and the two measured evidence objects the
    case narratives quote (baseline drift, three-zone reach)."""
    return {
        "overall": result["overall"],
        "overallMeasured": result["overallMeasured"],
        "results": [
            {
                "n": r["n"],
                "name": r["name"],
        ...
        "drift": result.get("baselineDrift"),
        "zones": result.get("zoneProfile"),
    }


def write_frontend_cases(results_by_name):
    """Emit frontend/scripts/core/case-studies-data.js: the trimmed
    before/after analyses behind docs/case-studies.md, in the shape
    case-studies.html renders. Written here, like the sample report data,
    so the docs JSONs and the in-app case studies cannot drift apart."""
    payload = {
        case_id: {
            "before": _trim_for_cases(results_by_name[before]),
            "after": _trim_for_cases(results_by_name[after]),
        }
        for case_id, before, after in CASE_PAIRS
    }
    path = os.path.join(
        OUT_DIR,
        "..",
    ...
    with open(path, "w", encoding="utf-8", newline="\n") as f:
        f.write(
            "/* Generated by docs/examples/generate_examples.py - do not"
            " edit by hand.\n"
            "   Real build_analysis output for the three before/after"
            " practice pairs\n"
            "   (docs/case-studies.md tells their stories);"
            " case-studies.html renders\n"
            "   the comparison. */\n"
        )
        f.write("window.VAHINI_CASE_STUDIES = ")
        json.dump(payload, f, indent=2, ensure_ascii=False)
        f.write(";\n")

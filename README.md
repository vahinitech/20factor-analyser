...
No install needed to judge what the analyser delivers:

- **[Example report](docs/example-report.md)** — a complete report walked
  through page by page, every number generated by the real engine.
- **[Case studies](docs/case-studies.md)** — three writers (sinking
  baseline, crowded words, single-zone letters), before and after the
  prescribed drills, scored by the real engine both times.
- **[Scoring criteria](docs/scoring-criteria.md)** — all 20 factors with
  their formulas, targets and band cut-offs, traced line by line to
  `backend/scoring.py`.

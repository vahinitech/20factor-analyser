...
### Browser account access and upload bounds

The browser offers an optional private access-key connection. Keys remain only in the current tab's memory and are cleared by disconnect or reload. Before each same-origin scan, `/api/v2/me` resolves the current server entitlement. Free requests include `text`; Pro requests include `text,inputs,coaching,evidence` for the detailed printable report. The key is sent only in an Authorization header to the same HTTPS origin (HTTP loopback is allowed for development), never to fallback hosts, storage or a URL. Redirects are rejected. This is operator-issued key access, not store billing, signup or account recovery.

Backend limits apply without nginx: request bodies are counted before multipart parsing (32 MiB including framing), individual uploads are limited to 30 MiB, and decoded images or first-page PDF renders must not exceed 24 million pixels. PDFs must contain 1–100 pages; only the first is analysed. Decoding and entitlement database lookups run in worker threads. Cache hits still pass the post-processing entitlement check. These bounds do not replace inference concurrency limits or per-customer quotas.

## Machine-readable contract

`backend/schemas/` holds JSON Schema (draft-07) files for every version-2 message. `backend/contract_schemas.py` validates a body from Python or the command line, and `backend/tests/test_schemas.py` checks in CI that the committed examples, every archived catalogue and the live builder output for Free, Pro and failed reports still match. See [the schema README](schemas/README.md) for the file list and how a client repository uses them.

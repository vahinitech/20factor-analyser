...
(function(global){
  ...
  function recommend(results){
    ...
    const valid=(Array.isArray(results)?results:[]).filter(f=>f && Number.isInteger(f.n) && Number.isFinite(f.score) && f.score>=0 && f.score<7 && !f.unmeasured && (f.conf==='measured'||f.conf==='estimated'||f.imuMeasured===true));
  ...
  function mount(host,results){
    ...
    const intro=document.createElement('p');intro.textContent=selected.length?'Selected from lower scores in this report. Start with one sheet and compare your next sample.':'There are no suitable lower measured factors to select from. Browse the starter guide or choose an exercise yourself.';section.append(intro);

...
2:{ honest:()=>`The order you build each letter in happens in time: a photo only shows the finished mark, so we cannot truly see it. The sensor pen records every stroke as you write.`,
    drill:'Trace guided stroke-order sheets slowly', next:'until a pen capture, treat this as a hint, not a score.' },
...
13:{ honest:()=>`Writing speed is invisible in a photo: we can only estimate it from how your strokes thin and thicken. The sensor pen times every stroke directly, 208 times a second.`,
    drill:'Write to a steady 1-2-3 count per word', next:'until a pen capture, treat this as a hint, not a score.',
...
14:{ honest:()=>`Pen pressure is invisible in a photo: we can only estimate it from ink darkness. Your ink ${'varies'}, which may mean uneven pressure. The sensor pen measures pressure directly.`,
    drill:'Aim for a light hand: about 20% of your usual force; if your fingertip whitens or the page embosses, ease off (a relaxed grip writes longer without tiring)', next:'until a pen capture, treat this as a hint, not a score.',
...
16:{ honest:()=>`Pen lifts happen in time: a photo cannot show them at all, so this is a neutral placeholder. The sensor pen counts every lift the moment it happens.`,
    drill:'Write whole words without lifting mid-word', next:'until a pen capture, this is not a score.',
